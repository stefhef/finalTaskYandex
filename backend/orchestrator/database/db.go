...
func GetUserPassword(access_token string) (string, error) {
	conn, err := pgx.Connect(context.Background(), "postgres://admin:root@postgres_container:5432/main")
	if err != nil {
		log.Fatal(err)
		return "", err
	...
	var password string
	err = conn.QueryRow(context.Background(), "SELECT password FROM users WHERE  = $1", access_token).Scan(&password)
	if err != nil {
		log.Fatal(err)
		return "", err
	}
...
func VerificateUser(access_token string, password string) bool {
	hashedPassword, err := GetUserPassword(access_token)
	if err != nil {
		log.Fatal(err)
		return false
	}
...
func AproveToken(access_token string) (string, error) {
	...
	if !VerificateUser(password, username) {
